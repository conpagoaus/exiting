...
exports.Manager = class {
  ...
  constructor(servers, options = {}) {
    ...
    this.servers =
      typeof servers[Symbol.iterator] === "function" ? [...servers] : [servers];
  ...
  async start() {
    ...
    try {
      ...
      if (aborted) {
        // Note that throw is not returned when aborted
  ...
  stop(options = {}) {
    Hoek.assert(
      this.state === "started",
      "Stop requires that server is started"
    );
  ...
  _unhandledError(type, err) {
    if (err instanceof exports.ProcessExitError) {
      // Ignore ProcessExitError, since we are already handling it
      return;
    }
    ...
    if (this.state === "stopping") {
      // Exceptions while stopping advance to error state immediately
      this.state = "errored";
    }
  ...
  _badExitCheck() {
    if (
      ...
      this._log(
        "Process exiting without stopping server (state == " + this.state + ")"
      );
    }
  }

  _setupExitHooks() {
    internals.addExitHook(
      "uncaughtException",
    ...
    for (const [event, graceful] of internals.signals) {
      const handler = graceful
        ? this._gracefulHandler.bind(this, event)
        : this._abortHandler.bind(this, event);
